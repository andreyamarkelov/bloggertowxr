// Tests for blogger.ParseReader and ResolveFeedPath using tiny fixtures (no real Takeout tree required).
package blogger
...
func TestParseReader_postsPagesAndSkipsComments(t *testing.T) {
	...
	wantHTML := "<p>Body\u00a0x</p>" // &nbsp; → Unicode NBSP after html.UnescapeString
...
func TestResolveFeedPath_singleBlog(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(blogs, "feed.atom"), []byte(minimalAtom), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if name != "OnlyBlog" || filepath.Base(filepath.Dir(path)) != "OnlyBlog" {
		t.Fatalf("got %s %s", name, path)
	}
}
